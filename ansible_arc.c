...
void handler_ArcPresetKey(s32 data) {
	...
	case 1:
		...
		case mArcCycles:
			...
			break;
		case mArcHello:
			flashc_memset8((void*)&(f.hello_state.preset), arc_preset, 1, true);
			init_hello();
			arc_leave_preset();
			resume_hello();
			break;
	...
	case 3:
		...
		case mArcCycles:
			...
			break;
		case mArcHello:
			flashc_memcpy((void *)&f.hello_state.c[arc_preset], &c, sizeof(c), true);
			flashc_memset8((void*)&(f.hello_state.preset), arc_preset, 1, true);
			arc_leave_preset();
			resume_hello();
			break;
...
void default_hello() {
uint16_t enc, led;
//code to light up all Arc LEDs to half brightness
...
void init_hello() {
	...
	arc_preset = f.hello_state.preset;
...
void handler_HelloKey(s32 data) {
	...
	case 0:
		if(key_count_arc[0]) {
			key_count_arc[0] = 0;
			monomeLedBuffer[enc*64 + led] = 5;
			// SHORT PRESS
		}
		// LONG UP
		else {
			monomeLedBuffer[enc*64 + led] = 14;
		}
		break;
...
void refresh_hello_config(void) {
